...
- Can restate this more compactly as:
...
```{r cum-loss-full, eval=TRUE, fig.cap="Comparison of cumulative loss over the entire sample period for conventional VAR and proposed Deep VAR.", out.width=300}
...
- To allow for a 'fairer' comparison we need to let Bayesian VAR, non-linear VAR and large econometric models enter into the horse race.

## Concluding remarks

- To the best of our knowledge we are the first to propose a framework for Deep VARs that barely deviates from the conventional framework.
- We show that relaxing the linearity constraint leads to a massive boost in modelling performance.
- To allow other researchers straight-forward access the methodology has been easily avialable through an R package:
...
- We continue to be in close contact with Eddie Gerba, research manager at the Bank of England, and plan to submit to an upcoming conference on ML for Monetary Policy.
...
## Cumulative loss for random walk

```{r, fig.cap="Cumulative mean squared error for naive (random walk) prediction.", out.width=300}
knitr::include_graphics("../www/cum_loss_full_random_walk.png")
```
