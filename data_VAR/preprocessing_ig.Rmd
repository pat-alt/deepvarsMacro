...
for (country in countries){
  country <- rbindlist(
    lapply(
      countries,
      function(c) {
        data_files_country <- data_files[grepl(c,data_files,ignore.case = TRUE)] # country-level data
        rbindlist(
          lapply(
            data_files_country,
            function(file_path) {
              country <- fread(file.path("data_VAR", file_path))
              setnames(country, colnames(country), c("date", "value"))
              variable <- file_path
              for (pattern in c(country,".csv","_")) {
                print(variable)
