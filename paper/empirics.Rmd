...
dates <- data.table(window=1:fcst[,max(window)], date=dt[(240+1):(.N-12),date])
setkey(fcst, window)
setkey(dates, window)
fcst <- dates[fcst]
```
...
library(ggplot2)
plot_data <- fcst[id %in% c(1,3),.(date=date,error=cumsum(sqerror)),by=.(variable,model,id)]
p <- ggplot(data = plot_data[model!="tv"]) +
...
ggsave("../www/rolling_window_cum_loss.png", plot=p, width = 10, height = 5)
```
