...
# Choosing lags:
max_lags <- 12
lags <- lag_order(dt[,-1], max_lag = max_lags)$p
# Deep VAR params:
num_units <- getOption("deepvar.model")$hidden_size
num_layers <- getOption("deepvar.model")$num_layers
dropout <- getOption("deepvar.model")$dropout
epochs <- 1200
```
...
```{r, eval=TRUE}
#Train test split
train_test_split <- train_val_test_split(dt[,-1])
train_data <- train_test_split$train
train_start <- dt$date[1]
train_end <- dt$date[nrow(train_data)]
val_start <- dt$date[nrow(train_data)+1]
val_end <- dt$date[nrow(train_data)+nrow(train_test_split$val)]
test_end <- tail(dt$date,1)
...
Tables \@ref(tab:rmse) shows the Root Mean Squared Error (RMSE) for the in-sample and the out-of-sample predictions of both the VAR model and the Deep VAR model. We can see that the RMSE for the Deep VAR is lower than for the conventional VAR for both the training data and the test data and for all time series. The fifth column of the table shows us the ratio between the RMSEs of the Deep VAR and the VAR: the lower the ratio, the better the Deep VAR compared to the VAR. With respect to the training sample, the RMSE of the Deep VAR model is consistently less than 75% of that of the conventional VAR reflecting to some extent the results of the previous sections. Turning to the test data, there is no evidence that the Deep VAR is more prone to overfitting than the VAR. For both industrial production and unemployment, the Deep VAR yields an RMSE that is around half the size of that produced by the VAR. For inflation and interest rate predictions the out-performance on the test data is less striking, but still fairly large.
...
w_size <- 240
n_ahead <- 12
fcst <- rolling_window_fe(dt[,-1],w_size = w_size, n_ahead = n_ahead, t=t)
dates <- data.table(window=1:fcst[,max(window)], date=dt[(w_size+1):(.N-n_ahead),date])
setkey(fcst, window)
setkey(dates, window)
fcst <- dates[fcst]
saveRDS(fcst, sprintf("../results/rolling_fcst_%s_%i_i.rds",t,w_size,n_ahead))
```

```{r, eval=TRUE}
fcst <- readRDS("../results/rolling_fcst_recursive.rds")
n_ahead <- fcst[,max(id)]
w_size <- dt[,.N]-fcst[,max(window)+max(id)]
```


```{r, fig.width=7, fig.height=6}
library(ggplot2)
plot_data_1 <- fcst[id == 1,.(date=date,error=cumsum(sqerror)),by=.(variable,model)][,id:=1]
plot_data_3 <- fcst[id <= 3,.(error=mean(sqerror)),by=.(variable,model,date)][,error:=cumsum(error),by=.(variable,model)][,id:=3]
plot_data_6 <- fcst[id <= 6,.(error=mean(sqerror)),by=.(variable,model,date)][,error:=cumsum(error),by=.(variable,model)][,id:=6]
plot_data_12 <- fcst[id <= 12,.(error=mean(sqerror)),by=.(variable,model,date)][,error:=cumsum(error),by=.(variable,model)][,id:=12]
plot_data <- rbind(plot_data_1,plot_data_3,plot_data_6, plot_data_12)
plot_data[,id:=factor(id)]
levels(plot_data$id) <-  c("=1","<=3","<=6","<=12")
p <- ggplot(data = plot_data[model!="TVAR"]) +
  geom_line(aes(x=date, y=error, color=model)) +
  facet_wrap(id ~ variable, scales = "free", ncol = 4) +
  scale_colour_discrete(name="Model") +
  labs(
    x = "",
    y = "Mean Squared Error (cumulative)"
  )
ggsave("../www/rolling_window_recursive_cum_loss.png", width=9, height=8)
```

```{r, eval=TRUE}
fcst <- readRDS("../results/rolling_fcst_3_240.rds")
n_ahead <- fcst[,max(id)]
w_size <- dt[,.N]-fcst[,max(window)+max(id)]
fcst[,model:=factor(model)]
levels(fcst$model) <- c("Deep VAR", "Random Walk", "Threshold VAR", "VAR")
```
...
plot_data[,id:=factor(id)]
levels(plot_data$id) <-  c("=1","<=3","<=6","<=12")
p <- ggplot(data = plot_data[model!="Threshold VAR"]) +
  geom_line(aes(x=date, y=error, color=model)) +
  facet_wrap(id ~ variable, scales = "free", ncol = 4) +
  scale_colour_discrete(name="Model") +
  labs(
...
ggsave("../www/rolling_fcst_240_12.png", width=9, height=8)
```

```{r, fig.width=10, fig.height=5}
library(ggplot2)
plot_data <- fcst[id %in% c(1,3),.(date=date,error=cumsum(sqerror)),by=.(id,variable,model)]
ggplot(data = plot_data[model!="TVAR"]) +
  geom_line(aes(x=date, y=error, color=model)) +
  facet_wrap(id ~ variable, scales = "free", ncol = 4) +
  scale_colour_discrete(name="Model") +
  labs(
    x = "",
    y = "Squared residual (cumulative)"
  )
```
...
Finally, we repeat the forecasting exercise above using a rolling window approach: we train our models on a window of `r w_size` months, compute and store `r n_ahead`-month ahead forecasts out of the training sample, roll the window one period forward and repeat the previous steps. This allows us to benchmark the different models in terms of their forecasting performance over the entire sample period. Once again forecasts are for now computed recursively: in other words, neural networks underlying the Deep VAR are not explicitly trained to forecast `r n_ahead`-steps ahead.

In Figure \@ref(fig:cum-rolling-recursive-full) we have plotted the cumulative loss incurred by each model: the different output variables are faceted across columns; each row corresponds to a different forecast horizon. For example, the panel in row 2 of column 3 shows the cumulative mean squared error incurred by each model for forecasts up to the 3-month horizon. 
...
```{r cum-rolling-recursive-full, eval=TRUE, fig.cap="Comparison of cumulative rolling-window forecasting error over the entire sample period for Deep VAR and benchmarks. Forecasts are computed recursively."}
knitr::include_graphics("../www/rolling_window_recursive_cum_loss.png")
```

<!-- As pointed out several times above, forecasting from Deep VAR does not strictly rely on recursion: instead of training the Deep VAR to produce a single output ($y_{i,t}$), we can instead let the final layer of the underlying neural networks map to $n$ outputs ($[y_{i,t},y_{i,t+n-1}]$). This effectively corresponds to training the Deep VAR directly for the purpose of $n$-step ahead forecasting. We now employ this approach. In addition (and only for the Deep VAR) we use a small subset of the training data as a validation set to guide a simple early stopping rule. That means the Deep VAR is actually trained on less observations than all other models. Nonetheless, since the underlying optimization in this case is done in the explicit context of forecasting we may expect the relative forecasting performance of the Deep VAR to increase further.  -->

<!-- Indeed the results in Figure \@ref(fig:cum-rolling-full) seem to confirm that expectation: the Deep VAR's relative performance is slightly better than before for all variables with the exception of CPI, which is somewhat worse than before. When trained explicitly for the purpose of forecasting, the Deep VAR outperforms conventional approaches for all variables. Once again out-performance is more pronounced at short forecast horizons. -->

<!-- ```{r cum-rolling-full, eval=TRUE, fig.cap="Comparison of cumulative rolling-window forecasting error over the entire sample period for Deep VAR and benchmarks. Neural networks underlying Deep VAR were trained on 12 outputs."} -->
<!-- knitr::include_graphics("../www/rolling_window_12_cum_loss.png") -->
<!-- ``` -->
