...
\begin{equation}
\begin{aligned}
&& \mathbf{y}_t&=\mathbf{c} + \mathbf{A}_1 \mathbf{y}_{t-1} + \mathbf{A}_2 \mathbf{y}_{t-2} + ... + \mathbf{A}_p \mathbf{y}_{t-p} + \mathbf{u}_t, && \mathbf{u}_t&\sim \mathcal{N}(\mathbf{0},\Sigma_u) (\#eq:redform)
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
&& \mathbf{y}_t&=\mathbf{A} \mathbf{Z}_{t-1} + \mathbf{u}_t (\#eq:sur)
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
&& y_{it}&=c_i+\sum_{m=1}^{p}\sum_{j=1}^{K}a_{jm}y_{jt-m}+u_{it}&&,&&\forall i=1,...,K (\#eq:single-var)
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
&& \mathbf{y}&=\mathbf{Z} \mathbf{\tilde{A}} + \mathbf{u}_t (\#eq:var-ols)
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
&& \mathbf{A}&=\mathbf{y}^T\mathbf{Z}(\mathbf{Z}\mathbf{Z}^T)^{-1} (\#eq:ols-sol)
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
&& y_{it}&=f_i\left(\mathbf{y}_{t-1:t-p};\theta\right)+v_{it} &&,&&\forall i=1,...,K (\#eq:single-dvar)
\end{aligned}
\end{equation}
...
Note that the input and output dimensions in \@ref(eq:single-dvar) are exactly the same as in the conventional VAR($p$) model (equation \@ref(eq:single-var)): $f_i$ maps from $\mathbf{y}_{t-1:t-p} \in \mathbb{R}^{Kp \times1}$ to a scalar. Our proposed plain-vanilla approach to Deep VAR models diverges as little as possible from the conventional approach: it boils down to simply modelling each of the univariate outcomes in \@ref(eq:single-dvar) as a deep neural network. We can restate this approach more compactly as 

\begin{equation}
\begin{aligned}
&& \mathbf{y}_t&= \mathbf{f}(\mathbf{y}_{t-1:t-p};\theta) + \mathbf{v}_t (\#eq:dvar)
\end{aligned}
\end{equation}
...
The notation in \@ref(eq:dvar) gives rise to a more unified and general approach to Deep VAR models that would treat the whole process as one single dynamical system to be modelled through one deep neural network $\mathbf{g}$:

\begin{equation}
\begin{aligned}
&& \mathbf{y}_t&= \mathbf{g}(\mathbf{y}_{t-1:t-p};\theta) + \mathbf{v}_t (\#eq:dvar-uni)
\end{aligned}
\end{equation}

This approach is in fact proposed and investigated by @verstyuk2020modeling in his upcoming publication. We decided to go with the approach in \@ref(eq:dvar) for two reasons: firstly, the link to conventional VAR models is made abundantly clear through this implementation and, secondly, we found that the equation-by-equation approach produces good modelling outcomes and is relatively easy to implement using state-of-the art software.
...
\begin{equation}
\begin{aligned}
&& f_i(\mathbf{y}_{t-1:t-p};\theta)&=f_i^{(H)}\left(f_i^{(H-1)}\left(...f_i^{(1)}\left(\mathbf{y}_{t-1:t-p}\right)\right)\right) (\#eq:mlp)
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
&& \mathbf{h}_t&=f(\mathbf{h}_{t-1},\mathbf{x}_t;\theta) (\#eq:hidden-state)
\end{aligned}
\end{equation}
...
&& \hat{y}_{it}&=c + \mathbf{v}^T\mathbf{h}_t (\#eq:rnn)
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
&& \mathbf{C}_{t}&=\mathbf{f}_t \odot \mathbf{C}_{t-1} + \mathbf{i}_t \odot \mathbf{\tilde{C}}_{t-1} (\#eq:lstm-update)
\end{aligned}
\end{equation}
...
&& \hat{y}_{it}&=c + \mathbf{v}^T\mathbf{h}_t (\#eq:lstm)
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
&& C(m)&= \log(\det(\hat\Sigma(m))) + \ell(m) (\#eq:ic)
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
&& \hat{p}&=\arg\min_{m\in\mathcal{P}}C(m) (\#eq:ic-min)
\end{aligned}
\end{equation}
