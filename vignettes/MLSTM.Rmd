...
pred_var <- deepvars:::predict.var_model(var_model)
deepvars:::plot.predictions(pred_var, y_true = y_true)
```
...
```{r}
rmse_var <- deepvars::rmse(var_model)[,model:="VAR"]
rmse_mlstm <- rmse(mlstm_model)[,model:="MLSTM"]
tab_rmse <- rbind(rmse_var, rmse_mlstm)
tab_rmse <- dcast(tab_rmse, variable ~ model, value.var = "value")
knitr::kable(
  tab_rmse, 
