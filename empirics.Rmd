...
for (l in layers){
  for (u in units){
    for (d in dropout){
      for (lag in lags){
        # Fit model
        deepvar_model <- deepvareg(train_data, lags=lag, num_units = u, epochs=100,     p_drop_out = d)
        y_true <- var_model$y_train
        # Train RMSE
        train_rmse <- rmse(deepvar_model)[,model:="var"][,sample:="train"][,layers:=l][,units:=u][,dropout:=d][,lags:=lag]
        # Test RMSE
        X_test <- prepare_test_data(train_test_split, lags=lags)$X_test
        y_test <- prepare_test_data(train_test_split, lags=lags)$y_test
        test_rmse <- rmse(deepvar_model, X=X_test, y=y_test)[,model:="var"][,sample:="test"][,layers:=l][,units:=u][,dropout:=d][,lags:=l]
        # Forecasts RMSE
        n_ahead <- 12
        y_true_fcst <- y_test[1:n_ahead,]
        fcst_dvar <- forecast(deepvar_model, n.ahead = n_ahead)
        fcst_rms <-rmsfe(fcst_dvar, y_true = y_true_fcs)
        # Forecasts correlations
        corr <- correlations <- correlations(fcst_var,fcst_dvar,y_true)$cor_dvar
        results <- rbind(train_rmse, test_rmse, fcst_rms, corr)
      }
    }
  }
}
