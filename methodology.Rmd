...
where $\mathbf{f}(\cdot)=(f_1(\cdot), f_2(\cdot),...,f_K(\cdot)^T$ is just the stacked vector of mappings to univariate outcomes described in \@ref(eq:single-dvar).
...
\begin{equation} 
\begin{aligned}
&& \mathbf{y}_t&= f(\mathbf{y}_{t-1:t-p};\theta) + \mathbf{v}_t \\
\end{aligned}
(\#eq:dvar-uni)
\end{equation}
...
These regulating gate layers include a **forget gate** $\mathbf{f}_t$, an **input gate** $\mathbf{i}_t$ and a **output gate** $\mathbf{o}_t$. Each of them are vector-values sigmoid functions whose elements $\bf{f}_{it},\bf{i}_{it},\bf{o}_{it}$ are bound between $0$ and $1$. Their individual purposes are implied by their names: faced with $\bf{h}_{t-1}$ and $\mathbf{y}_{t-1:t-p}$, the forget gate regulates how much of each individual unit in $\bf{C}_{t-1}$ is retained. Then the input gate regulates which units of $\bf{C}_{t-1}$ should be updated and to what candidate values $\bf{\tilde{C}}_{t-1}$. Using the previous two steps the actual update is performed according to the following rule

$$
\begin{equation} 
\begin{aligned}
&& \bf{C}_{t}&=\bf{f}_t \odot \bf{C}_{t-1} + \bf{i}_t \odot \bf{\tilde{C}}_{t-1}\\
\end{aligned}
(\#eq:lstm-update)
\end{equation}
$$

where $\odot$ indicates the element-wise product. Finally, the output gate acts like a filter on $\bf{C}_{t}$: the new hidden state is computed as $\bf{h_t}=\bf{o}_t \odot \tanh (\bf{C_t})$ where as before we use the hyperbolic tangent as our activation function.^[For a clear and detailed exposition see @olah2015understanding.] Formally, we can summarize the LSTM neural network underlying our Deep VAR framework as follows

$$
\begin{equation} 
\begin{aligned}
&& \mathbf{f}_t&=\sigma \left( \mathbf{b}_f + \mathbf{W}_f\bf{h}_{t-1} + \mathbf{U}_f\bf{h}_{-1} \right) \\
&& \mathbf{i}_t&=\sigma \left( \mathbf{b}_i + \mathbf{W}_i\bf{h}_{t-1} + \mathbf{U}_i\bf{h}_{-1} \right) \\
&& \mathbf{o}_t&=\sigma \left( \mathbf{b}_o + \mathbf{W}_o\bf{h}_{t-1} + \mathbf{U}_o\bf{h}_{-1} \right) \\
&& \bf{C}_t&=\mathbf{f}_t \odot \mathbf{C}_{t-1} + \mathbf{i}_t \odot \tanh \left( \mathbf{b}_C + \mathbf{W}_C\bf{h}_{t-1} + \mathbf{U}_C\bf{h}_{-1} \right) \\
&&\bf{h_t}&=\bf{o}_t \odot \tanh (\bf{C_t}) \\
&& \bf{\hat{y}}_t&=\mathbf{c} + \bf{V}\bf{h}_t\\
\end{aligned}
(\#eq:lstm)
\end{equation}
...
which is best understood when read from top to bottom. Once again we have simplified the notation we have omitted the layer index in \@ref(eq:rnn). The same notation as before applies.
