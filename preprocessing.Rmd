...
countries <- c("US") # add more as more data available
dt <- rbindlist(
  lapply(
    countries,
    function(country) {
...
```{r}
fwrite(dt, "data_VAR/merged_raw.csv")
```
...
```{r}
dt <- fread("data_VAR/merged_raw.csv") # load the raw merged data
```
...
fwrite(dt, "data_VAR/preprocessed.csv")
fwrite(dt_l, "data_VAR/preprocessed_tidy.csv")
```
