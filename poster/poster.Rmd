...
1. **Threshold VAR**: We have since added Threshold VAR (TVAR) to the analysis: . We find that a plain-vanilla Deep VAR trained for 500 epochs clearly outperforms TVAR (see Figure \@ref(fig:cum-loss-full)).
2. **Others**: We'd love to hear your suggestions for other benchmarks.

## Inference

In general, we fear that inference is a whole lot more difficult for Deep VAR.
...
The conventional VAR is a special case of Seemingly Unrelated Regressions (SUR) and can be efficiently estimated through equation-by-equation ordinary-least-squares (OLS). We apply the same approach to Deep VAR without proving that this is still efficient. Since we achieve good modelling performance and our approach is methodologically close to the VAR, we are not altogether to worried. Still, our approach to estimating Deep VAR lacks a solid theoretical foundation.
