...
lags <- 3
# Prepare X and y:
mlstm_data <- prepare_mlstm_data(dt[year(date)<2020], type="standard")
```

Prepare the model ...

```{r}
mlstm_ <- mlstm(mlstm_data) # prepare MLSTM
```
...
```{r}
# Train-test split:
list2env(train_test_split(var_data_lstm), envir = environment())
print(dim(X_train))
print(dim(y_train))
print(dim(X_test))
print(dim(y_test))
