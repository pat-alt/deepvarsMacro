...
lags <- 3
# Prepare X and y:
var_data <- prepare_data(dt[year(date)<2020,.SD,.SDcols=model_vars], lags = lags, standardize = TRUE)
...
epochs <- 10
mlstm_ <- fit(X_train, y_train, X_test, y_test, mlstm_, epochs = epochs) # fit models
```

```{r}
pred <- predict(X_train, mlstm_)
p <- plot(pred, y_true = y_train)
```

```{r}
pred <- predict(X_test, mlstm_)
