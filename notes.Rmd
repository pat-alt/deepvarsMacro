...
```{r}
my_pred <- forecast(vars[[country]], n.ahead=n_ahead)
plot(my_pred, history = 100)
```
...
deepvar_model <- deepvareg(dt, lags=lags, n_units = hidden_units) 
```
...
```{r}
fcst <- forecast(mlstm_, n.ahead = 10)
plot(fcst, history = 24)
```
...
```{r}
# Train-test split:
list2env(train_test_split(mlstm_data), envir = environment())
print(dim(X_train))
print(dim(y_train))
print(dim(X_test))
print(dim(y_test))
...
# Fit models
mlstm_ <- fit(
  mlstm_, X_train, y_train, X_test, y_test,  epochs = epochs, verbose=0
) 
```

```{r}
pred <- predict(mlstm_, X_train)
plot(pred, y_true = y_train)
```

```{r}
pred <- predict(mlstm_, X_test)
plot(pred, y_true = y_test)
```
