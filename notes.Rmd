...
First, let us check if the VAR is stable. For this, I first run the reduced-form VAR with a conventional (?) choice of $p=12$ lags (reflecting `r lags/12` year given the monthly frequency).
...
For the Deep VAR we vary the number of hidden layers (depth), the number of hidden neurons per layer (width) and the drop out rate (regularization). 
...
```{r}
pred <- predict(var_model)
plot(pred, y_true = y_true)
```

```{r}
pred <- predict(deepvar_model)
plot(pred, y_true = y_true)
```
...
```{r}
fcst <- forecast(var_model, n.ahead=n_ahead)
plot(fcst, history = 100)
```

```{r}
fcst <- forecast(deepvar_model, n.ahead = n_ahead)
plot(fcst, history = 100)
```
